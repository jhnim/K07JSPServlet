...
try{
	...
	/*  
	파일 다운로드를 위한 응답헤더 설정
	: 웹브라우저가 인식하지 못하는 컨텐츠타입(MIME)을 응답헤더레 설정하면
	다운로드 창을 보여주게된다. (웹브라우저에 따라 다운로드창의 형태는 달라진다.)
	*/
	response.reset() ;
	...
	if(!skip){
		...
		/*  
		input스트림으로 파일내용을 읽어와서 output스트림으로 웹브라우저에
		출력한다. 이때 웹브라우저는 해당 파일의 MIME타입을 인식하지 못하므로
		다운로드창을 통해 파일을 다운로드 시키게된다.
		*/
