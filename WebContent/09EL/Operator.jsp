...
<body>
	...
	<!--  
		...
		만약
			해당페이지?myNumber=30		=> 결과 30 출력
			해당페이지?myNumber=		=> 0으로 간주되어 20출력
	...
	<!--  
		...
		산술연산에서는 0으로 간주하고 연산을 진행하지만 비교연산은 null과의 비교자체가 불가능하기 때문이다.
	-->
	...
	<%
	/*  
	EL에서는 JSP에서 선언한 변수는 직접 사용할 수 없다. 값이 출력되지 않고 null로 인식하게 된다.
	JSP에서 변수를 EL에서 사용할 수없는이유는 EL은 4가지 영역에 저장된 속성을  사용하기 때문이다.
	JSTL도 동일한특성을 가지고 있다.
	*/
	...
	<!--  
		JSP에서 선언한 변수를 EL에서 사용해야 할 경우에는 JSTL의 set태그를 이용해서 변수를 선언한다.
		JSP에서 선언후 바로 사용하려면 영역에 저장해야 한다.
	-->
	...
	<!--  
		...
		버전을 확인한 후 사용여부를 결정해야 한다.
		EL은 전통적으로 값을 표현(출력)하는 용도로 사용되어 졌으므로 표현용으로만 사용하는 것이 좋다.
	-->
	<h3>EL변수에 값 할당</h3>
	...
	<!--  
		...
		즉 자동형변환되어 출력된다. 나눈셈을 위한 / 연산자 대신 div,
	...
	<ul>
		<!--  
			fnum 과 snum은 영역에 저장된 데이터이므로 Object형으로 저장된다.
			따라서 비교시 객체상태에서 비교가 이루어지게된다. 100과 90은 실제 숫자의 비교가 이루어진다.
		-->
		<li>\${fnum > snum } : ${fnum > snum }</li><!-- 결과: false -->
		<li>\${100 > 90 } : ${100 > 90 }</li><!-- 결과: true -->
		
		<!--  
			Java에서는 문자열을 비교할때 equals()로 비교하지만 EL에서는 ==의 형태로 비교한다.
		-->
	...
	<h3>EL의 논리연산자</h3>
	<ul>
		<li>\${5>=5 && 10!=10 } : ${5 ge 5 and 10 ne 10 }</li><!-- 참&& 거짓 => 거짓 -->
		<li>\${5>6 || 10<100 } : ${5 gt 6 or 10 lt 100 }</li><!-- 거짓 || 참 => 참 -->
	</ul>
	...
	<c:set var="lengthZero" value="<%=lengthZero %>" />
	<c:set var="sizeZero" value="<%=sizeZero %>" />
