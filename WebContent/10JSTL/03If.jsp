...
<body>
	...
	- else문이 없기 때문에 다중조건일때는 표현이 어렵다. 이때는 주로 choose문을 사용한다.
	형식]
		test : 조건
	...
	<!--  
		if(numVar%2==0) 와 동일한 조건으로 , 해당 조건문은 true를 반환하므로 태그 사이의 문장이 출력된다.
	...
	<!--  
		...
		아래와 같이 참의 조건, 거짓의 조건으로 각각 블럭을 만들어준다.
	-->	
	<h3>문자열 비교</h3>
	...
	<!--  
	- test속성에 EL식이 아닌 일반 값이 들어가면 무조건 false를 반환한다.
	- EL식이더라도 {} 양쪽에 공백이 들어가면 무조건false를 반환한다.
	...
	<c:if test="tRuE" var="result">
		TRUE(true) 대고문자 구분없음. 항상참임<br>
	</c:if>
	\${result } : ${result }<br>
	
 	<h3>EL식으로 조건판단시 {}양쪽에 공백이 있으면 무조건false로 판단됨</h3>
	<c:if test="${true }" var="result">
	...
	<c:if test="${not empty param.user }">
		...
		<br />
		<!--  
		첫번째 if문에서 의 결과에 따라 두번째 if문은 반대의 결과를 가지게 되는 구조로 작성되었다.
		즉 loginResult가 true가 된다면 두번쨰 if문은 false값으로 바뀌게 된다.
		-->
