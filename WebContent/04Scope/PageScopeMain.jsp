...
<%
/*
JSP의 모든 영역(페이지, 리퀘스트, 세션, 어플리케이션)에는 모든 타입의 객체를 저장할 
수 있다. Object타입으로 자동형변환되어 저장되고, 사용시에는 각 객체로 형변환 후 사용하면 된다.
...
<body>
	...
	<%
	/*
	page영역에 저장된 속성을 읽어옴. 영역에 저장될때는 Object형으로 자동형변환되므로, 
	...
	*/
	...
	<ul>
		...
		<li>MemberDTO타입2 : 아이디:<%=m2.getId() %>,
			비번:<%=m2.getPass() %> 이름:<%=m2.getName() %> 가입일:<%=m2.getRegidate() %></li>
	</ul>
	...
	<%-- 
		외부파일을 인클루드 하는 것은 해당 파일을 원본그대로 현재문서에
		포함시킨 후 컴파일되므로 include지시어를 사용하는 경우 같은 페이지로 
		취급되어 페이지 영역이 공유된다.
	--%>
