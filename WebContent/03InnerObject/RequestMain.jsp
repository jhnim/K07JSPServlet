...
<body>
	...
	<h2>파라미터 및 값전달</h2>
	<%
	//한글을 전송하는경우 서버환경에 따라 깨질 수 있으므로 인코딩 후 전달하는 것이 안전하다.
	...
	<form method="post" name="regFrm" action="RequestParameter.jsp" onsubmit="return checkForm(this);">
		...
		성별 :
			...
			<input type="radio" name="sex" value="woman" checked="checked">여자ㅣ
		<br />
		자기소개 :
		<textarea rows="10" cols="30" name="self_intro">나는 부유한 집안에서 태어나서 대충 사라씸...</textarea>
		<!--  
		textarea는 태그와 태그 사이에 컨텐츠가 들어가므로  정렬을 위해 줄바꿈을 사용하게 되면 필요없는 공백이 포함될 수 있으므로 주의해야 한다.
		-->
		<br />
			<input type="submit" value="POST방식">
	</form>
